<template>
  <el-card shadow="naver">
    ...
    <el-table  :data="trademarkList" style="margin:10px 0" border>
      ...
      <el-table-column label="品牌操作">
        <template #="{row}">
          ...
          <el-popconfirm icon="Delete" icon-color="black" :title="`确定删除${row.tmName}嘛?`" width="250px"
    ...
    <!-- 修改添加弹窗 -->
    <el-dialog v-model="dialogVisible" :title="trademarkForm.id?'修改品牌':'添加品牌'" width="500">
      <el-form ref="formRef" :model="trademarkForm" :rules="rules" style="width:80%">
        ...
        <el-form-item prop="logoUrl" label="品牌LOGO" label-width="100px">
          ...
          <el-upload v-loading="loading" class="avatar-uploader" action="/api/admin/product/fileUpload"
            ...
            <img style="height:178px" v-if="trademarkForm.logoUrl" :src="trademarkForm.logoUrl" class="avatar" />
      ...
      <template #footer>
        ...
        <el-button type="primary" size="small" @click="confirm" :disabled="loading?true:false">确定</el-button>
      </template>
    </el-dialog>
  </el-card>
</template>
